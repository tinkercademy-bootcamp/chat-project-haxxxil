...
void handle_connections(int sock, int port) {
  ...
  //
  // #Answer - no, since this way the CPU wastes time by scheduling this program
  // and also the accept system call is probably expensive
  while (true) {
...
int main() {
  ...
  //
  // #Answer - we can just have start_listening and the on_socket part feels extra.
